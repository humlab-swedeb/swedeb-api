...
def test_word_trends_with_base_model(corpus):
    df = corpus.get_word_trend_results(
        search_terms=["debatt", "riksdagsdebatt"],
        filter_opts={},
        start_year=1900,
        end_year=2000,
    )
...
def test_word_trends_api_with_filter(client):
    search_term = "att,och"
    response = client.get(
        f"{version}/tools/word_trends/{search_term}?parties=9&genders=2&from_year=1960&to_year=1970"
    )
    json = response.json()
    first_result = json["wt_list"][0]
    print(json)
...
def test_word_trend_corpus_with_filters(corpus):
    ...
    assert (
        "1963" in wt.index
    )  # year without result for att for 1960-test corpus should be included
    columns = wt.columns
    for c in columns:
        # 9 corresponds to S
        assert "S" in c
...
def test_summed_word_trends(corpus):
    ...
    df = corpus.get_word_trend_results(
        search_terms=["debatt", "riksdagsdebatt", "debatter"],
        filter_opts={},
        start_year=1960,
        end_year=1961,
    )
    ...
    df = corpus.get_word_trend_results(
        search_terms=["debatt"], filter_opts={}, start_year=1960, end_year=1961
    )
...
def test_eu_debatt(corpus):
    df = corpus.get_word_trend_results(
        search_terms=["EU-debatt"], filter_opts={}, start_year=1900, end_year=3000
    )
    print(df.head())
    df_small = corpus.get_word_trend_results(
        search_terms=["eu-debatt"], filter_opts={}, start_year=1900, end_year=3000
    )
    print(df_small.head())
...
def test_frequent_words(corpus):
    word_hits_non_descending = corpus.get_word_hits(
        "katt*", n_hits=10, descending=False
    )
    ...
    df = corpus.get_word_trend_results(
        search_terms=word_hits_descending,
        filter_opts={},
        start_year=1900,
        end_year=3000,
    )
    ...
    df = corpus.get_word_trend_results(
        search_terms=word_hits_non_descending,
        filter_opts={},
        start_year=1900,
        end_year=3000,
    )

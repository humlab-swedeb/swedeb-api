# Data Used by a Running Swedeb API Instance
...
Tables loaded from the `.db` file (as defined in `config.yml → mappings.tables`):
...
| `vrt/*.vrt.gz`                                    | VRT source files                      | No                      | Used to build the CWB corpus; not read at runtime       |
 ...
 - `tagged_frames/config*.yml`, `version`, `metadata_version` are version/config marker files, and not used (build artifacts)
 - `Makefile`, `*.log`, `opts/`, `protocols.txt`, `tag.sh`, `.env` are build tooling and logs, and not used (pipeline artefacts)

"""N-gram archive service — prepare and execute async n-gram archive generation."""
...
# N-gram columns exposed in archive output (excludes internal identifiers)
_ARCHIVE_COLUMNS: list[str] = ["ngram", "window_count", "documents"]

_TICKET_ROW_ID = "_ticket_row_id"


class NGramsArchiveService:
    """Thin service that converts a ready n-gram Feather artifact into a bulk archive."""
    ...
    def prepare(
        ...
        *,
        ...
        """Validate the source n-gram ticket and create an archive ticket.

        Raises:
            ResultStoreNotFound: if the source ticket is missing or expired.
            ValueError: if the source ticket is not in a ready or partial state.
        """
    ...
    def execute_archive_task(
        ...
        *,
        ...
        """Serialize the n-gram Feather artifact and mark the archive ticket ready or failed."""
        logger.info(f"Starting NGrams execute_archive_task for archive ticket {archive_ticket_id}")

        try:
            archive_ticket: TicketMeta = result_store.require_ticket(archive_ticket_id)
        except ResultStoreNotFound:
            logger.warning(f"NGrams archive ticket {archive_ticket_id} not found; nothing to update")
            return
        except ResultStoreCapacityError:
            logger.warning(f"Result store capacity error loading NGrams archive ticket {archive_ticket_id}")
            return
        ...
        try:
            ...
            logger.info(f"NGrams archive ticket {archive_ticket_id} ready ({len(data)} rows)")

        except ResultStoreCapacityError:
            logger.warning(f"Result store capacity error for NGrams archive ticket {archive_ticket_id}")
            return
        except ResultStoreNotFound as exc:
            logger.warning(f"Required resource not found for NGrams archive ticket {archive_ticket_id}: {exc}")
            result_store.store_error(archive_ticket_id, message=f"Required resource not found: {exc}")
        except Exception as exc:  # pylint: disable=broad-except
            logger.exception(f"Error executing NGrams archive ticket {archive_ticket_id}: {exc}")
            result_store.store_error(archive_ticket_id, message="Failed to generate n-gram archive")
    ...
    def _write_zip_csv(self, data: pd.DataFrame, dest: Path) -> None:
        with zipfile.ZipFile(str(dest), "w", zipfile.ZIP_DEFLATED) as zf:
            with zf.open("ngrams_data.csv", "w") as entry:
